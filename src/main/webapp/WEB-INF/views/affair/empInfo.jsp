...
//					  formatoptions: { srcformat: 'ISO8601Long', newformat: 'Y-m-d' },  
	...
	function initEvent() {
		...
		$('#checkUserIdDuplicationButton').on("click", function() { 
			...
			$.ajax({ 
				type : 'POST',
				url : '${pageContext.request.contextPath}/affair/checkUserIdDuplication.do' ,
				data : {
		...
		$('#batchEmpDetailSaveButton').on("click", function() {
			...
			if( confirmStatus == true ) {
				...
				$.ajax({ 
					type : 'POST',
					url : '${pageContext.request.contextPath}/affair/batchListProcess.do' ,
					async :false,
					data : {
	...
	function showEmpListGrid() {
		...
		$.ajax({ 
			type : 'POST',
			url : '${pageContext.request.contextPath}/affair/searchAllEmpList.do' ,
			data : {
	...
	function showCodeDialog(grid, rowid, iCol, divisionCodeNo, title) {
		...
		$("#codeGrid").jqGrid({
			...
			url : "${pageContext.request.contextPath}/logisticsInfo/codeList.do",
			datatype : "json",
			jsonReader : {
				root : "detailCodeList"
			},
