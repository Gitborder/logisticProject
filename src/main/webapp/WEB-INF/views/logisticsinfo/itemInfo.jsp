...
<div>
    <h5>📋 품목 목록조회</h5>
    <button id="itemgroupBttton">품목 조회</button>
    <button id="deliverableContractSearchButton">품목 상세조회</button>
    <button id="deleteitemgroupBttton">품목 삭제</button>
...
<script>
  ...
  const itemgroupBtn = document.querySelector("#itemgroupBttton"); //품목그룹 조회
  const deleteitemgroupBtn = document.querySelector("#deleteitemgroupBttton"); //품목그룹 삭제

  const deliverableContractSearchBtn = document.querySelector("#deliverableContractSearchButton"); //품목상세목록 조회
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
 ...
 /*    getRowNodeId: function(data) {
  ...
  //목록상세조회 이벤트
  deliverableContractSearchBtn.addEventListener("click", () => {  //품목 목록조회
     let selectedRows = clientInformationGridOptions.api.getSelectedRows();
     console.log(selectedRows);
       if (selectedRows == "") {
         Swal.fire("알림", "선택한 행이 없습니다.", "info");
