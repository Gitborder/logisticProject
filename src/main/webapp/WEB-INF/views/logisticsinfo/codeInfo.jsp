...
<html>
		...
		<article class="estimate">
		    <div class="estimate__Title">
		        ...
		        <div class="menuButton">
		            <button  id="customerList" >코드조회</button>
	...
	<script>
	  ...
	  const codeDetailInsertBtn = document.querySelector('#codeDetailInsertButton');
	 ...
	 //-----------------------------------------코드 조회------------------------------------------// 
	  ...
	  customerList.addEventListener("click", () => { 
	     ...
	     let xhr = new XMLHttpRequest();
	       xhr.open('POST', '${pageContext.request.contextPath}/logisticsinfo/searchCodeList.do' +
	           "?method=findCodeList", true)
	...
	codeDetailInsertButton.addEventListener("click", () => {
      ...
      let xhr = new XMLHttpRequest();
        xhr.open('POST', "${pageContext.request.contextPath}/logisticsinfo/searchCodeDetilList.do"+
	...
	let newcodeRowValues=[];
	  ...
	  batchSaveButton.addEventListener("click", () => {
	    ...
	    estDetailGridOptions.api.forEachNode(function(eachRow){
            if (eachRow.data.detailCodeName == '') {
               Swal.fire({
                  text: "상세코드이름이 작성되지 않은 데이터가 있습니다.",
                  icon: "info",
               })
               return;
            }
	    ...
	    Swal.fire({
	      ...
	      let xhr = new XMLHttpRequest();
	      xhr.open('POST', "${pageContext.request.contextPath}/logisticsinfo/codechange.do?method=addCodeInFormation&newCodeInfo="
