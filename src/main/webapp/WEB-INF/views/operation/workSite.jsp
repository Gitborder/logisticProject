  ...
  completeBtn.addEventListener('click', () => { // 검사 및 제작완료   P_WORK_SITE_COMPLETION 프로시저
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let xhr = new XMLHttpRequest();
        xhr.open('POST', '${pageContext.request.contextPath}/operation/workCompletion.do' +
...
//-----------------------------------작업장 조회버튼------------------------------------//  
  ...
  const workSiteLog = () => {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '${pageContext.request.contextPath}/operation/workSiteLog.do' +
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        console.log(txt);
        workSiteLogGridOptions.api.setRowData(txt.gridRowJson.gridRowJson);
      }
    }
  }
